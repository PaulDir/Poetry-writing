...
poem_generator = Poem()
dictionary,_ = poem_generator.data_set()

# x_data, y_data = poem_generator.next()
def Train():
    input_size = output_size = len(poem_generator.wordToid) + 1
    ...
    learning_rate = tf.train.exponential_decay(0.01, global_step, poem_generator.tranSet_batch_num, 0.9, staircase=True)
    ...
    with tf.Session() as session:
        ...
        for epoch in range(epochNum):
            ...
            epoch_mean_cost = 0
            for batch in range(poem_generator.tranSet_batch_num):
                x_data, y_data = poem_generator.next()
                ...
                print("current epoch %d, current batch is %d, mean cost : %2.8f, learning rate: %2.8f, global step : %d"
                    %(epoch, batch, c, lr, gs))
            epoch_mean_cost = epoch_mean_cost / poem_generator.tranSet_batch_num
            print("="*80)
            if epoch != 0:
